...
from bayesianmodels_new import BayesianModel, GibbsState, ArrayTree
...
from New_kernel_1 import Discontinuous_multiple, Discontinuous_multiple_params_hyper
# from uicsmodels.gaussianprocesses.gputil import update_gaussian_process_cov_params_num, update_gaussian_process_cov_params_lv
from gputil_new import sample_prior, sample_predictive, update_gaussian_process, update_gaussian_process_cov_params, update_gaussian_process_mean_params, update_gaussian_process_obs_params
from gputil_new import update_gaussian_process_cov_params_num, update_gaussian_process_cov_params_lv, update_gaussian_process_cov_params_num_hyper, update_gaussian_process_cov_params_lv_hyper, update_gaussian_process_hyper_params
from gputil_new import update_gaussian_process_diag_params
...
class FullGPModel(BayesianModel):
    ...
    def inference(self, key: PRNGKey, mode='gibbs-in-smc', sampling_parameters: Dict = None, poisson = False):
        ...
        return super().inference(key, mode, sampling_parameters, poisson)
...
class FullLatentGPModel(FullGPModel):
    ...
    def gibbs_fn(self, key: PRNGKey, state: GibbsState, temperature: Float= 1.0, **mcmc_parameters):
        ...
        if len(cov_params):
            ...
            key, subkey = jrnd.split(key)
            scale = 10
            new_cov_fn = Discontinuous_multiple(jk.RBF(), temp=((scale**(1-temperature)-1)/(scale-1)))
...
class FullMarginalGPModel(FullGPModel):
    ...
    # def predict_y_particle(self, key: PRNGKey, x_pred: Array, particles):
    #     """Samples from the posterior predictive distribution
...
class FullMarginalGPModelhyper(FullMarginalGPModel):
    """The marginal Gaussian process model.
    ...
    def init_fn(self, key: Array, num_particles: int = 1):
        ...
        priors_new = dict(zip(names, priors_flat))
        key, subkey = jrnd.split(key)
        hyper_sample = priors_new['hyper_pp'].sample(seed=subkey, sample_shape=num_particles)
        samples.append(hyper_sample)
        
        for prior, name in zip(priors_flat, names):
            key, subkey = jrnd.split(key)
            if name == 'num':
                samples.append(prior._sample_n(subkey, num_particles, hyper_sample))
            elif name == 'hyper_pp':
                pass
            else: 
    ...
    def logprior_fn(self) -> Callable:
        ...
        def logprior_fn_(state: GibbsState):
            ...
            logprob = 0
            priors_flat, priors_treedef = tree_flatten(self.param_priors, lambda l: isinstance(l, (Distribution, Bijector)))
            values_flat, vals_treedef = tree_flatten(position)
            ...
            for value, dist, name in zip(values_flat, priors_flat, names):
                if name == 'num':
                    logprob += jnp.sum(dist.log_prob(value, vals_new['hyper_pp'])) 
...
class FullMarginalGPModelhyper_mult(FullMarginalGPModel):
    ...
    def init_fn(self, key: Array, num_particles: int = 1):
        ...
        # jax.debug.print('OG_names {}', OG_names)
        num_flat, num_priors_treedef = tree_flatten(self.param_priors['kernel'], lambda l: isinstance(l, Poisson_Process_added.Poisson_Process_hyper))
        # jax.debug.print('num_flat {}', num_flat)
        num_distr = [obj for obj in num_flat if isinstance(obj, Poisson_Process_added.Poisson_Process_hyper)]
        # num_test = [obj for obj in num_flat if isinstance(obj, Poisson_Process_added.Poisson_Process_hyper) or obj == 0.0]
        # num_names = [x for x, obj in zip(num_priors_treedef.node_data()[1], num_test) if isinstance(obj, Poisson_Process_added.Poisson_Process_hyper)
        priors_flat, priors_treedef = tree_flatten(self.param_priors, lambda l: isinstance(l, (Distribution, Bijector)))
        # jax.debug.print('priors_flat {}', priors_flat)
        children = priors_treedef.children()
        leaves = [x.node_data()[1] for x in children]
        # jax.debug.print('leaves {}', leaves)
        # names = [item for sublist in leaves for item in sublist]
        # jax.debug.print('names {}', names)

        # num_names = [x for x, obj in zip(names, priors_flat) if isinstance(obj, Poisson_Process_added.Poisson_Process_hyper)]
        # jax.debug.print('num names {}', num_names)
        hyper_flat, hyper_priors_treedef = tree_flatten(self.param_priors['hyper'], lambda l: isinstance(l, (Distribution, Bijector)))
        # hyper_names = [x for x in hyper_priors_treedef.node_data()[1]]
        # awyuidc
        # jax.debug.print('hyper_flat {}', hyper_flat)
        samples = list()
        ...
        if isinstance(self.param_priors['hyper'], dict):
            ...
            for _, hyper_dist in enumerate(hyper_flat):
                key, subkey = jrnd.split(key)
                # print(hyper_dist.sample(seed=subkey, sample_shape=num_particles))
        ...
        if isinstance(self.param_priors['kernel'], dict):
            ...
            for i, num_s in enumerate(num_distr):
                key, subkey = jrnd.split(key)
                # print(jnp.sort(num_s._sample_n(subkey, num_particles, hyper_sample[i]), axis = 1))
        ...
        for sup_name in OG_names:
            ...
            if sup_name == 'kernel':
                if isinstance(self.param_priors[sup_name], dict):
                    for name, prior in dict(sorted(self.param_priors[sup_name].items())).items():
                        if isinstance(prior, Poisson_Process_added.Poisson_Process_hyper):
        ...
        #         # jax.debug.print("sample: {}", prior._sample_n(subkey, num_sample, num_particles))
        #         samples.append(prior._sample_n(subkey, num_sample, num_particles))
        #     elif name == 'variance':
        #         samples.append(prior._sample_n(subkey, num_sample, num_particles))
        #     elif name == 'hyper_pp':
        #         samples.append(hyper_sample)
    ...
    def logprior_fn(self) -> Callable:
        ...
        def logprior_fn_(state: GibbsState):
            ...
            # names = [item for sublist in leaves for item in sublist]
            # num_names = [x for x, obj in zip(names, priors_flat) if isinstance(obj, Poisson_Process_added.Poisson_Process_hyper)]
            # num_test = [obj for obj in num_flat if isinstance(obj, Poisson_Process_added.Poisson_Process_hyper)]
            # num_names = [x for x, obj in zip(num_priors_treedef.node_data()[1], num_test) if isinstance(obj, Poisson_Process_added.Poisson_Process_hyper)]
                ...
                if sup_name == 'kernel':
                    if isinstance(self.param_priors[sup_name], dict):
                        ...
                        for name, dist, value in zip(sorted_dict.keys(), sorted_dict.values(), kernel_vals.values()):
                            if isinstance(dist, Poisson_Process_added.Poisson_Process_hyper):
            ...
            # for value, dist, name in zip(values_flat, priors_flat, names):
            #     if name == 'num':
            #         logprob += jnp.nansum(dist.log_prob(value, vals_new['hyper_pp'])) \
...
class FullLatentGPModel_new(FullLatentGPModel):
    """The latent Gaussian process model.
    ...
    def gibbs_fn(self, key: PRNGKey, state: GibbsState, temperature: Float= 1.0, **mcmc_parameters):
        ...
        if len(cov_params):
            ...
            new_cov_fn = Discontinuous_multiple(jk.RBF(), temp=((scale**(1-temperature)-1)/(scale-1)))
            key, subkey = jrnd.split(key)
            sub_state, sub_info = update_gaussian_process_cov_params_num(subkey, self.X,
                                       position['f'],
                                       mean_fn=self.mean_fn,
                                       cov_fn=new_cov_fn,
                                       mean_params=mean_params,
            ...
            scale = 10
            new_cov_fn = Discontinuous_multiple(jk.RBF(), temp=((scale**(1-temperature)-1)/(scale-1)))
            sub_state, sub_info = update_gaussian_process_cov_params_lv(subkey, self.X,
                                       position['f'],
                                       mean_fn=self.mean_fn,
                                       cov_fn=new_cov_fn,
                                       mean_params=mean_params,
                                       cov_params=cov_params,
                                       hyperpriors=self.param_priors['kernel'])
            position['kernel']['lengthscale'] = sub_state['lengthscale'] 
            position['kernel']['variance'] = sub_state['variance'] 
        #
        if len(likelihood_params):
            # Sample parameters of the likelihood using: 
            # p(\phi | y, f) \propto p(y | f, phi)p(phi)
...
class FullLatentGPModelhyper_mult(FullLatentGPModel):
    ...
    def init_fn(self, key, num_particles=1):
        ...
        for sup_name in OG_names:
            ...
            if sup_name == 'diag':
                for name, prior in dict(sorted(self.param_priors[sup_name].items())).items():
                    samples.append(prior._sample_n(subkey, num_particles))
        ...
        # num_sample = priors_new['num']._sample_n(subkey, hyper_sample, num_particles)
    ...
    def loglikelihood_fn_new(self, position, temperature) -> Float:
        ...
        # b = 1
        # def exp_decay(temp):
        #     return limit/(jnp.exp(b*limit) + jnp.exp(b*1)) * jnp.exp(b*temp)+ limit/(jnp.exp(b*limit) + jnp.exp(b*1)) * jnp.exp(b*1)
        # def nothing(temp):
        #     return temp
        X = self.X
        ...
        cov_params = position.get('kernel', {}) 
        jitter =  1e-5
        # temp = (scale**(temperature)-1)/(scale-1)
        # temp = jax.lax.cond(temperature > limit, exp_decay, nothing, temperature)
        # temp = ((jnp.sum(~jnp.isnan(position['kernel']['num']))/position['kernel']['num'].shape[0])**2)*(1- temperature)
        temp = 0
    ...
    def logprior_fn_new(self, position):
        ...
        for sup_name in OG_names:
            if sup_name == 'kernel':
                ...
                sorted_dict = dict(sorted(self.param_priors[sup_name].items()))
                for name, dist, value in zip(sorted_dict.keys(), sorted_dict.values(), diag_vals.values()):
                    logprob += jnp.sum(dist.log_prob(value))
        # # position = getattr(state, 'position', state)
        # logprob = 0
        # priors_flat, priors_treedef = tree_flatten(self.param_priors, lambda l: isinstance(l, (Distribution, Bijector)))
        ...
        # vals_new = dict(zip(names, values_flat)) ## gets rid of 'f'
        # dist_new = dict(zip(names, priors_flat))


        # # jax.debug.print("vals: {}", vals_new)
        # # jax.debug.print("dist: {}", dist_new)

        # # jax.debug.print("vals new: {}", vals_new)
        # # jax.debug.print("dist new: {}", dist_new)
        # # jax.debug.print(names[0] + names[1] +names[2] +names[3] +names[4])
    ...
    def gibbs_fn(self, key: PRNGKey, state: GibbsState, temperature: Float= 1.0, poisson = True, **mcmc_parameters):
        ...
        def logdensity_(key, new_pos, old_pos, temperature):
                ...
                ### log_likelihood should be p(f|params) -> MVN (with temp scaling)
                ### log prior will be on all params
        ...
        def poisson_process_step_new(key, position, temperature):
            ...
            #     value = remove_cov_param(value, index-1)
            #     value = jax.lax.cond(index != len(num_params), add_cov_param, new_nothing, value, index-1, dist)
            #     value = jax.lax.cond(index != len(num_params), add_cov_param, new_nothing, value, index, dist)
            #     return value
            ...
            #     value = jax.lax.cond(index != -1, remove_cov_param, nothing, value, index-1)
            #     value = add_cov_param(value, index-1, dist)
            #     # if index != 0:
            ...
            # for value, dist, name in zip(values_flat, priors_flat, names):
            #     value = jax.lax.cond(p, add_cov, remove_cov, value, index, dist)
            #     # value = jax.lax.cond(p, add_cov, remove_cov, value, index-1, dist)
            #     new_position['kernel'][name] = value
        ...
        diag_params = position.get('diag', None)

        loglikelihood_fn_ = lambda f_: temperature * jnp.sum(self.likelihood.log_prob(params=likelihood_params, f=f_, y=self.y))

        key, subkey = jrnd.split(key)
        # scale = 10
        # new_cov_fn = Discontinuous_multiple_params_hyper(jk.RBF(), temp=((scale**(1-temperature)-1)/(scale-1)))
        position['f'], f_info = update_gaussian_process(subkey,
                                                        position['f'],
                                                        loglikelihood_fn_,
                                                        self.X,
        ...
        if diag_params is not None:
            sub_state, sub_info = update_gaussian_process_diag_params(subkey, self.X,
                                        position['f'],
                                        mean_fn=self.mean_fn,
                                        cov_fn=self.cov_fn,
                                        mean_params=mean_params,
                                        ...
                                        hyperpriors=self.param_priors['diag'],
                                        temp = temperature)
            position['diag'] = sub_state
        ...
        if len(cov_params):
            ...
            # p(theta | f, psi) \propto p(f | psi, theta)p(theta)
            # scale = 1000000
            # temp = 0.1 * ((scale**(1-temperature)-1)/(scale-1))
            # temp = 0
            # new_cov_fn = Discontinuous_multiple_params_hyper(jk.RBF(), temp=temp)
            # new_cov_fn = Discontinuous_multiple_params_hyper(jk.RBF(), temp=(1-temperature))
            if isinstance(self.param_priors['kernel'], dict):
                ...
                # values_flat, vals_treedef = tree_flatten(cov_params)
                # cov_params['num'] = num
                ...
                # cov_params_subset_new = sub_state
                # cov_params_subset_new['num'] = cov_params['num']
                sub_state['num'] = cov_params['num']
                position['kernel'] = sub_state
                # values_flat, _ = tree_flatten(cov_params_subset)
                ...
                for i, (kernel, kernel_val) in enumerate(zip(self.param_priors['kernel'], position['kernel'])):
                    ...
                    key, subkey = jrnd.split(key)
                    # jax.debug.print(type(cov_params).__name__)
                    ...
                    # cov_params_subset = {j:cov_params[j] for j in cov_params if j!='num'}
                    # cov_params_subset_new = sub_state
                    # cov_params_subset_new['num'] = cov_params['num']
                    sub_state['num'] = cov_params['num']
                    position['kernel'][i] = sub_state
                    # jax.debug.print('params', position['kernel'][i])
