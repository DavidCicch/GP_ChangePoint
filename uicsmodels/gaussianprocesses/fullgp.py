...
from uicsmodels.gaussianprocesses.gputil import sample_prior, sample_predictive, update_gaussian_process, update_gaussian_process_cov_params, update_gaussian_process_cov_params_new, update_gaussian_process_mean_params, update_gaussian_process_obs_params
...
class FullLatentGPModel(FullGPModel):
    ...
    def __init__(self, X, y: Optional[Array] = None,
        ...
        super().__init__(X, y, cov_fn, mean_fn, priors, sigma, **kwargs)      
    ...
    def gibbs_fn(self, key: PRNGKey, state: GibbsState, temperature: Float= 1.0, **mcmc_parameters):
        ...
        # jax.debug.print('loglikelihood of f: {}', loglikelihood_fn_(position['f']))
        if len(mean_params):
            # Sample parameters of the mean function using: 
            # p(psi | f, theta) \propto p(f | psi, theta)p(psi)      

            key, subkey = jrnd.split(key)
            sub_state, _, sub_info = update_gaussian_process_mean_params(subkey, self.X,
                                       position['f'],
                                       mean_fn=self.mean_fn,
                                       cov_fn=self.cov_fn,
                                       mean_params=mean_params,
        ...
        if len(cov_params):
            ...
            # new_cov_fn = Discontinuous_multiple(jk.RBF(), temp=0.1*(1-temperature))

            # white_params = dict(variance = 1-temperature) 
            # new_params = [cov_params, white_params]
            sub_state, sub_density, sub_info = update_gaussian_process_cov_params(subkey, self.X,
                                       position['f'],
                                       mean_fn=self.mean_fn,
                                       cov_fn=new_cov_fn,
                                       mean_params=mean_params,
            ...
            # position['cov_log_dens']['loss'] = sub_density.flatten()
        ...
        if len(likelihood_params):
            ...
            key, subkey = jrnd.split(key)
            sub_state, _, sub_info = update_gaussian_process_obs_params(subkey, self.y,
                                       position['f'],
