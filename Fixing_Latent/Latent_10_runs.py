...
pd_all = []
f_pred_all = jnp.zeros((num_particles, 100, num_runs))
y_pred_all = jnp.zeros((num_particles, 100, num_runs))

for i in range(num_runs):
  print(i)
  key, gpm_key = jrnd.split(key)
  # mgp_particles, _, mgp_marginal_likelihood = gp_latent.inference(gpm_key,
  #                                                                 mode='gibbs-in-smc',
...
jnp.save(f"particles_all_10_{num_mcmc_steps}_scale1_1000000_var3_0_new_test.npy", pd_all)
jnp.save(f"f_pred_10_{num_mcmc_steps}_scale1_1000000_var3_0_new.npy_test", f_pred_all)
jnp.save(f"y_pred_10_{num_mcmc_steps}_scale1_1000000_var3_0_new.npy_test", y_pred_all)